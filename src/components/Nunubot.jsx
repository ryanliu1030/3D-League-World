...
function Nunubot(props) {
  const group = useRef();
  const { nodes, materials, animations } = useGLTF(
    'src/static/models/nunubot.glb'
  );
  const { actions } = useAnimations(animations, group);
  const [, getKeyboardInput] = useKeyboardControls();
  const [animation, setAnimation] = useState('nunu_idle01.anm');
  ...
  useFrame(() => {
    const { forward, backward, leftward, rightward, laugh, snowball } =
      getKeyboardInput();
    if (forward || backward || leftward || rightward) {
      if (snowball) {
        setAnimation('nunu_spell2.anm');
      } else {
        setAnimation('nunu_run01.anm');
      }
    } else if (laugh) {
      setAnimation('nunu_skin04_laugh01.anm');
      if (!laughSound.playing()) {
        laughSound.play();
      }
    } else {
      setAnimation('nunu_idle01.anm');
    }
  });
